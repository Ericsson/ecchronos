...
def print_schedules(schedules, max_lines):
    schedule_table = [["NodeID", "JobID", "Keyspace", "Table", "Status", "Repaired(%)",
...
def print_repairs(repairs, max_lines=-1):
    repair_table = [["NodeID", "JobID", "Keyspace", "Table", "Status", "Repaired(%)",
...
def _convert_repair(repair):
    entry = [repair.host_id, repair.job_id, repair.keyspace, repair.table, repair.status,
             repair.get_repair_percentage(), repair.get_completed_at(), repair.repair_type]
    return entry


def _convert_schedule(schedule):
    entry = [schedule.node_id, schedule.job_id, schedule.keyspace, schedule.table, schedule.status,
