...
def step_validate_expected_show_table_header(context, keyspace, table, repair_type):
    ...
    assert re.match("NodeID : .*", strip_and_collapse(table_info[0])), "Faulty Id '{0}'".format(table_info[0])
    assert strip_and_collapse(table_info[1]) == "Keyspace : {0}".format(keyspace), "Faulty keyspace '{0}'".format(
        table_info[1]
    )
    assert strip_and_collapse(table_info[2]) == "Table : {0}".format(table), "Faulty table '{0}'".format(table_info[2])
    assert re.match(
        "Status : (COMPLETED|ON_TIME|LATE|OVERDUE)", strip_and_collapse(table_info[3])
    ), "Faulty status '{0}'".format(table_info[3])
    assert re.match(
        "Repaired\\(%\\) : \\d+[.]\\d+", strip_and_collapse(table_info[4])
    ), "Faulty repaired(%) '{0}'".format(table_info[4])
    assert re.match("Completed at : .*", strip_and_collapse(table_info[5])), "Faulty repaired at '{0}'".format(
        table_info[5]
    )
    assert re.match("Next repair : .*", strip_and_collapse(table_info[6])), "Faulty next repair '{0}'".format(
        table_info[6]
    )
    assert re.match(
