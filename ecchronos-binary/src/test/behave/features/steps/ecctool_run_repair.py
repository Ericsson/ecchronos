...
@when("we run repair for keyspace {keyspace} and table {table}")  # pylint: disable=not-callable
...
@when("we run repair for keyspace {keyspace}")  # pylint: disable=not-callable
def step_run_repair_keyspace(context, keyspace):
    run_ecc_run_repair(context, ["--keyspace", keyspace])
    handle_repair_output(context)


@when("we run repair")  # pylint: disable=not-callable
...
@when("we run local repair for keyspace {keyspace}")  # pylint: disable=not-callable
def step_run_local_repair_for_keyspace(context, keyspace):
    run_ecc_run_repair(context, ["--keyspace", keyspace, "--local"])
    handle_repair_output(context)


@when("we run local repair")  # pylint: disable=not-callable
