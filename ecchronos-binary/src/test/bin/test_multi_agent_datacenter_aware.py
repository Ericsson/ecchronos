...
def test_lock_concurrency(install_cassandra_cluster, test_environment):
    ...
    try:
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            logger.info(
                f"On Demand Jobs created with exit code: ecchronos-agent-dc1: {exit_code_ecc_dc1}, "
                f"ecchronos-agent-dc2: {exit_code_ecc_dc2}"
            )
            ...
            try:
                ...
                logger.error("Timeout: Repairs did not complete within 5 minutes")
...
def test_lock_concurrency_scheduled(install_cassandra_cluster, test_environment):
    ...
    try:
        with ThreadPoolExecutor(max_workers=2) as executor:
            # No on-demand trigger here; scheduled repairs should run based on schedule.yml
            # mounted via EcchronosConfig for each instance.
            try:
                wait_for_repairs_completion(executor)
            except Exception as e:
                logger.error("Timeout: Scheduled repairs did not complete within 10 minutes")
...
def wait_for_repairs_completion(executor):
    ...
    logger.info(
        f"Waiting for repairs... DC1: {sum(1 for s in ecc_dc1_statuses if s == 'COMPLETED')}/{len(ecc_dc1_statuses)}, "
